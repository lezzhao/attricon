...
import { createGenerator, loadIconCss } from '../packages/core'
...
describe('test', async () => {
  ...
  it('transform', async () => {
    const icon = await createGenerator({ prefix: 'i' })
    const icon1 = await createGenerator({ prefix: 'i' })
    const icon2 = await createGenerator({ prefix: 'i' })
